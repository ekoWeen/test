...
<body>
    ...
    <div class="container">
        <div class="row mt-3">
            <div class="col">
                <h2>Artikel sebelum Agustus 2019'</h2>
            </div>
        </div>
        <div>
            <p>Artikel yang terbit sebelum Agustus 2019: </p>
            <?php
            foreach ($profile as $row) {
                ...
                foreach ($article as $list) {
                    $title = $list["title"];
                    $dates = $list["published_at"];

                    // echo ($dates);
                    // echo "<br>";

                    $pattern = "2019-08";
                    $formating = date_create($dates);
                    $ym = date_format($formating, "Y-m");
